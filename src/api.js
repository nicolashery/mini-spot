...
// Hack, we should be requiring these here
...
api.init = function(options, cb) {
  ...
  // Hack
  // We handle session persist elsewhere, so pass in this fake localStore
  var localStore = {
    getItem: function() { return token; },
    setItem: function(key, token) {
      // Called on everytoken refresh, so update db and localStorage
  ...
  tidepool.initialize(function(err, auth) {
    ...
    if (auth) {
      ...
      api.user.get(function(err, user) {
        ...
        // Hack, AuthActions expects a token here
        api.token = token;
  ...
  tidepool.getViewableUsers(userId, function(err, permissions) {
    ...
    async.map(patientIds, getPatient, function(err, patients) {
      ...
      patients = _.map(patients, function(patient) {
        return _.assign(patient, {permissions: permissions[patient.userid]});
      });
